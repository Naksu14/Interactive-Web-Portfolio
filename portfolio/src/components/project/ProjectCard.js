...
const ProjectBlock = ({
  ...
  return (
    <div
      className={`mt-5 mb-20 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 grid-rows-3 gap-4 px-0 md:px-4 md:px-8 lg:px-16 xl:px-32`}
    >
      <div
          className={`${
            reverse ? "order-1 lg:order-2" : "order-1"
          } col-span-1 md:col-span-2 lg:col-span-3 row-span-3 flex justify-center`}
        >
          <div className="w-[90%] h-[350px] sm:h-[400px] md:h-[450px] lg:h-[500px] xl:h-[500px] bg-[#18161F] rounded-lg p-4 border-t border-white border-b border-gray-400">
      ...
      <div
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3">
          {techStack.map((tech, index) => (
            <div
              key={index}
              className="bg-[#AA897F] rounded-xl px-3 py-2 flex flex-row items-center justify-center gap-2 text-xs sm:text-sm"
            >
              <img
